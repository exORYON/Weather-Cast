...
function getCity() {
  ...
  request.open('GET',
    'https://ip-api.io/json?api_key=e55e4ed9-e9fb-4e7f-b17d-eb27c2b51505',
  );
      ...
      <div class='weather'>
        <h1 class='weather__title'>Текущая погода в городе ${city}</h1>
          <div class='weather__cast'>
            Температура:
              <span class='nums'>${currentWeather.main.temp}°</span><br>
            Ощущается как:
              <span class='nums'>${currentWeather.main.feels_like}°</span><br>
            Минимальная температура за день:
              <span class='nums'>${currentWeather.main.temp_min}°</span><br>
            Максимальная температура за день:
              <span class='nums'>${currentWeather.main.temp_max}°</span><br>
            Текущее давление:
              <span class='nums'>${currentWeather.main.pressure} гП</span><br>
            Влажность:
              <span class='nums'>${currentWeather.main.humidity}%</span><br>
            Видимость:
              <span class='nums'>${currentWeather.visibility} метров</span><br>
            Скорость ветра:
              <span class='nums'>${currentWeather.wind.speed} м/с</span><br>
            Облачность:
              <span class='nums'>${currentWeather.clouds.all}%</span><br>
          </div>
        <h3>
          Данные, согласно с сайтом
        ...
        <h1 class='weather__title'>Current weather in ${city}</h1>
          <div class='weather__cast'>
            ...
            Max. day temperature:
              <span class='nums'>${currentWeather.main.temp_max}°</span><br>
            Current pressure:
              <span class='nums'>${currentWeather.main.pressure} gP</span><br>
            Humidity:
              <span class='nums'>${currentWeather.main.humidity}%</span><br>
            Visibility:
              <span class='nums'>${currentWeather.visibility} meters</span><br>
            Wind speed:
              <span class='nums'>${currentWeather.wind.speed} m/s</span><br>
            Clouds:
              <span class='nums'>${currentWeather.clouds.all}%</span><br>
          </div>
        <h3>Info from
...
function getWeather() {
  const apiKey = '10cc54419fe99e60c1fd5c08247978a4';

  let requestURL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${apiKey}`;
  const request = new XMLHttpRequest();
